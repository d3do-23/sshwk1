...
def get_string_features(path,hasher):
    ...
    pe = pefile.PE(path)
    print(path)
    if hasattr(pe,"DIRECTORY_ENTRY_IMPORT")!=False:
    	for imp in pe.DIRECTORY_ENTRY_IMPORT:
            if imp.imports[0].name != None:
                import_features[imp.imports[0].name.decode()] = 1
                print(imp.imports[0].name.decode())
    hashed_features = hasher.transform([import_features])
    ...
    print("Extracted {0} strings from {1}".format(len(import_features),path))
